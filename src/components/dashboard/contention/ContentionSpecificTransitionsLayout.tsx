...
const ContentionSpecificTransitionsLayout: React.FC<ContentionSpecificTransitionsLayoutProps> = ({ jobId, selectedActivityPair }) => {
    const [sourceActivity, destinationActivity] = selectedActivityPair.split(' - ');
    const overviewData = useFetchData(`http://154.56.63.127:5000/wt_overview/${jobId}/contention/${sourceActivity}/${destinationActivity}`);
    ...
    const waitingTimeOptions = {
        ...
        series: [{
            type: 'pie',
            data: [
                ['Contention WT', overviewData.wt_sum],
