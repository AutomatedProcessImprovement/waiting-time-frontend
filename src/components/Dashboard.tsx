...
const onDownload = (type: number, jobId: string) => {
    switch (type) {
        case 0:
            window.location.href = `http://154.56.63.127/assets/results/${jobId}/event_log_transitions_report.csv`;
            break;
        case 1:
            axios({
                url: `http://154.56.63.127/assets/results/${jobId}/event_log_transitions_report.json`,
                method: 'GET',
                responseType: 'blob'
            }).then(response => {
